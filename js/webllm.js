...
const ForgeWebLLM = {
  ...
  listModels() {
    return (this.lib && this.lib.prebuiltAppConfig && this.lib.prebuiltAppConfig.model_list) || [];
  },
  ...
  fillSelect() {
    ...
    sel.innerHTML = use.map((m) => {
      const gb = m.vram_required_MB ? (m.vram_required_MB / 1024).toFixed(1) + " GB VRAM" : "small";
  ...
  async warmupSelect() {
    try {
      ...
      else this.setStatus("WebLLM library ready. Pick a small model and load weights (one-time download).", 0);
    } catch (err) {
  ...
  async load(requested) {
    ...
    if (!this.secureOk()) throw new Error("WebLLM needs a secure context (HTTPS or localhost).");
    if (!this.gpuOk()) throw new Error("This browser has no WebGPU. Use Chrome or Edge 113+.");
    this.loading = true;
    this.ready = false;
    this.lastError = "";
    try {
      ...
      this.modelId = id;
      this.setStatus("Downloading " + id + " into this browser\u2026", 3);
      ...
      this.engine = await lib.CreateMLCEngine(id, {
        initProgressCallback: (p) => {
          ...
          this.setStatus((p.text || "Loading weights") + " \u00b7 " + pct + "%", pct);
        }
      });
      this.ready = true;
      this.setStatus("Ready \u00b7 " + id + " \u00b7 runs offline in this tab", 100);
    } catch (err) {
  ...
  async chat(messages) {
    ...
    const out = await this.engine.chat.completions.create({
      messages,
      temperature: 0.4,
      max_tokens: 260
    });
